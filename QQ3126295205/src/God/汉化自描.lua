local Script_Title = `汉化阿尔宙斯 <font color="rgb(255, 0, 0)">|</font> 子弹追踪，自动瞄准`
...
local toggleBtn
toggleBtn = addToggle("自动瞄准『😏不推荐会乱瞄，不是电脑不建议开启』", function(state)
	enabled = state
end, false)

local toggleTeamCheckBtn
toggleTeamCheckBtn = addToggle("子弹跟踪『推荐手机电脑都可以用』", function(state)
	teamcheck = state
end)

local aimPartCombo
aimPartCombo = addComboBox("瞄准部位", {"头", "身体"}, function(selection)
	aimpart = selection
end)
